...
export default async function AdminOrdersPage() {
  if (!await isAdmin()) return <div className="mx-auto my-16 max-w-4xl px-4"><h1 className="mb-6 text-2xl font-bold">Admin sign-in</h1><AdminLogin /></div>;
  ...
  return <div className="mx-auto my-16 max-w-5xl px-4">
    <div className="flex justify-between"><h1 className="text-2xl font-bold">Order management</h1><form action={logoutAdmin}><button className="underline">Sign out</button></form></div>
    <p className="my-4">Demo payments and fulfilment. Showing the most recent 200 orders.</p>
    {!orders.length && <p>No orders yet.</p>}
    {orders.map(order => <section key={order.id} className="my-4 rounded border p-4">
    ...
    {inventoryWarning && <p role="status" className="mb-4 text-amber-800">{inventoryWarning}</p>}
