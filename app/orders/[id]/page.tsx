...
const paymentNames = {
  "simulated-paid": "Demo payment approved",
  "simulated-pending": "Demo payment pending",
  "simulated-refunded": "Demo payment refunded",
  "simulated-cancelled": "Demo payment cancelled",
};
    ...
    <PageContainer>
      ...
      >
        <div className="flex gap-4">
          <span
            className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-white ${isCancelled ? "bg-danger" : isPending ? "bg-warning" : "bg-success"}`}
          >
            {isCancelled ? (
              <svg
                aria-hidden="true"
          ...
          <div>
            ...
            <p className="mt-1">
              {isCancelled
                ? "This demo order will not be fulfilled."
                : isPending
                  ? "The simulated payment is pending. You can follow its progress below."
                  : `Thanks, ${order.customer.name}. Your demo order has been placed.`}
            </p>
            <p className="mt-3 text-sm text-text-muted">
              No real payment was taken and no items will be shipped.
            </p>
          </div>
        </div>
      ...
      <div className="grid items-start gap-8 lg:grid-cols-5">
        <div className="grid gap-6 lg:col-span-3">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-text-muted">
                  {paymentNames[order.paymentStatus]}
                </p>
              </div>
          ...
          <Card className="p-4 sm:p-6">
            ...
            <address className="not-italic leading-7">
              ...
              <p className="mt-2 bold">{order.customer.email}</p>
            </address>
          </Card>
          ...
          <Card className="p-4 sm:p-6">
            ...
            <ol className="grid gap-0">
              {(order.events ?? []).map((event, index, events) => (
                <li
                ...
                >
                  <div className="flex flex-col items-center">
                    <span
                      className={`mt-1 h-3 w-3 rounded-full ${event.status === "cancelled" ? "bg-danger" : "bg-primary"}`}
                    />
                  ...
                  <div className="pb-5">
                    <p className="font-semibold">{statusNames[event.status]}</p>
        ...
        <OrderSummary
          quote={order.quote}
          className="lg:sticky lg:top-32 lg:col-span-2"
        />
      </div>
